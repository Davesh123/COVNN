...
training_set = train_datagen.flow_from_directory('3Class/training_set', target_size = (size, size), batch_size = 2, class_mode = 'categorical', color_mode='grayscale')

#No modifications are performed on the test_set
test_datagen = ImageDataGenerator(rescale = 1./255)

#assign the variable test_set with the data stored from this path
test_set = test_datagen.flow_from_directory('3Class/test_set', target_size = (size, size), batch_size = 2, class_mode = 'categorical', color_mode='grayscale')
...
model.fit(x = training_set, validation_data = test_set, epochs = 54, verbose=2, callbacks=[es])

#The model is saved for further use with prediction or statistics
model.save('saved_model/3Class') 
